...
export default function piSpecExtension(pi: ExtensionAPI): void {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		const sections = [
			`[PI SPEC CONTEXT]\nUse project-layer specs in ${SPEC_ROOT}/<feature>/ with brainstorm.md, requirements.md, design.md, and tasks.md. Brainstorm is a required gate before spec creation: identify user intent, ask one question at a time, propose an approach, and only then create requirements/design/tasks. Each gate has its own approval and its own Status line: requirements-draft → requirements-approved, design-draft → design-approved, tasks-draft → tasks-approved. Never bundle approvals across phases — a single "approved"/"go"/"yes" reply only advances the current gate. After the user approves a phase, update that file's \`Status:\` line to the matching \`*-approved\` value, then stop and ask for the next gate's approval explicitly. Do not draft the next phase in the same turn that you draft (or get approval for) the current phase. Keep implementation traceable to checked tasks and requirement IDs. Before implementing, verify tasks.md is explicitly approved by the user and no longer tasks-draft (run spec_validate with phase: "implementation"). Use the bundled spec skills when the request is about creating, refining, or implementing a spec.`,
		];
